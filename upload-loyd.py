...
class SubversionVCS(VersionControlSystem):
  ...
  def GetBaseFileStatus(self, filename):
    ...
    # the correct status for a file.
      ...
      if dirname not in self.svnls_cache:
        ...
        if self.options.svn_explicit_branches:
          ...
          cmd = ["svn", "list", self.svn_source_url + separator + dirname + "@" + self.rev_start or "."]
        else:
  ...
  def GetBaseFile(self, filename):
    status = self.GetBaseFileStatus(filename)
    base_content = None
    new_content = None
    ...
    if status[0] == "A" and status[3] != "+":
      ...
      if self.options.svn_explicit_branches:
        # the file won't be present in the source branch, so we get the mime-type from the target version
        mimetype = RunShell(["svn", "propget", "svn:mime-type", self.svn_target_url + "/" + filename + "@" + self.rev_end],
                          silent_ok=True)
      ...
      if is_binary and self.IsImage(filename):
        ...
        except iIOError:   # Ignore missing local image file (this can happen if the source rev is not HEAD)
          pass
